	...
	export default{
		...
		methods:{
			...
			async __init(){
				...
				((this.canvasW-hW) / 2) + hH + uni.upx2px(180)))
				//绘制虚线
				this.drawDashLine(this.ctx,uni.upx2px(20),(((this.canvasW-hW) / 2) + hH + uni.upx2px(240)),(this.canvasW-uni.upx2px(20)),(((this.canvasW-hW) / 2) + hH + uni.upx2px(240)),5)
				//左边实心圆
				...
				this.ctx.setFillStyle('#858585');
				this.ctx.fillText('长按识别图中“虔易”小程序',(((this.canvasW-hW) / 2) + uni.upx2px(34)),(((this.canvasW-hW) / 2) + hH + uni.upx2px(300)))
				//底部广告
				...
				this.ctx.drawImage(qrcodeImg.path,uni.upx2px(384),(((this.canvasW-hW) / 2) + hH + uni.upx2px(264)), uni.upx2px(156), uni.upx2px(156))
				//延迟渲染
				setTimeout(()=>{
					this.ctx.draw(true,()=>{
						uni.hideLoading()
