<template>
	<view class="main">
		<view class="top-background">
			...
			<view class="top-account-balance">
				<view class="balance-content">
					<view class="title">补贴账户(元)</view>
					<view class="accout">{{accoutAmount.RechargeAmount}}</view>
				</view>
			</view>
		</view>
		...
		<view class="allowance-interest">
			<view class="interest-content">
				...
				<view class="right">
					<view class="cont">
						<p>每日转化消费</p>
						<view class="button" @click="toIntegral">
							去看看
						</view>
					</view>
				</view>
		...
		<view class="account-details">
			<view class="details-content" v-for="(item,index) in RechargeAmountData" :key="index">
				<view class="middle" >
					<view class="left">
...
import {request} from '@/api/request.js'	
	export default{
		...
		onReachBottom() {
			if(this.RechargeAmountData.length<this.Index*10){
		...
		methods:
		{
			getSubSidyData(callBack){
				request('API_GetList_LogsUserProfit',{user_id:this.userInfo.user_id,LP_Type:0,AmountType:"Recharge",pageSize:10,index:this.Index}).then(res=>{
					if(res.length<10){
						this.loadStatus = 'nomore'
					}
					this.RechargeAmountData = [...this.RechargeAmountData,...res]
					callBack && callBack()
				})
			},
			toIntegral(){
				uni.navigateTo({
					url:`Integral?user_id=${this.userInfo.user_id}&accoutAmount=${this.accoutAmount.IntegralAmount}`
				})
			},
			toTransfer(){
				uni.navigateTo({
